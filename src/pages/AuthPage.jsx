...
export default function AuthPage({ onAuth, onShowPrivacy, onShowTerms, onForgotPassword }) {
  ...
  const switchMode = (m) => { setMode(m); setError(''); setInfo(''); setPassword(''); setConfirm(''); setAgreed(false); setShowPw(false); setShowConfirmPw(false); setAwaitingConfirmation(false); setResendMsg('') }
  ...
  if (awaitingConfirmation) return (
    <div style={PageWrap}>
      <div style={CardStyle}>
        ...
        <p style={{ ...Muted, marginBottom: 24 }}>Click the link in the email to activate your account, then come back and sign in. Check your spam folder if it's not in your inbox.</p>
        ...
        <button onClick={handleResend} disabled={resendLoading} style={{ ...BtnFull, marginBottom: 10, opacity: resendLoading ? 0.7 : 1 }}>
    ...
    <div style={PageWrap}>
      <div style={CardStyle}>
        ...
        <button onClick={handleGoogle} disabled={googleLoading}
          style={{ width: '100%', padding: '10px 16px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--bg-input)', color: 'var(--text)', cursor: googleLoading ? 'wait' : 'pointer', fontFamily: 'inherit', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 16, fontWeight: 500 }}>
          {GOOGLE_ICON}
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ position: 'relative' }}>
            <input style={{ ...I, paddingRight: 44 }} type={showPw ? 'text' : 'password'}
              placeholder="Password" value={password}
              onChange={e => setPassword(e.target.value)} onKeyDown={handleKey}
              autoComplete={mode === 'login' ? 'current-password' : 'new-password'} />
            <button onClick={() => setShowPw(v => !v)} style={EyeBtn} title={showPw ? 'Hide password' : 'Show password'} tabIndex={-1}>
              {showPw ? '🙈' : '👁️'}
            </button>
          </div>
        </div>
        ...
        {mode === 'signup' && (
          <>
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input style={{ ...I, paddingRight: 44 }} type={showConfirmPw ? 'text' : 'password'}
                ...
                <button onClick={() => setShowConfirmPw(v => !v)} style={EyeBtn} tabIndex={-1}>
                  {showConfirmPw ? '🙈' : '👁️'}
                </button>
              </div>
            </div>
        ...
        <button style={{ ...BtnFull, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}
...
const PageWrap = { minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-page)', padding: '1rem' }
const CardStyle = { width: '100%', maxWidth: 400, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '2.5rem 2rem' }
...
const EyeBtn = { position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, padding: 0, lineHeight: 1 }
const BtnFull = { width: '100%', padding: '10px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontFamily: 'inherit', fontWeight: 500, marginBottom: 4 }

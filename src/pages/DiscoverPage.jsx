...
// Browse: genre/sort catalog discovery, alongside (not replacing) the
// trending view above. Trending answers "what's hot right now" — this
// answers "show me horror movies" — a real gap when the only other way to
// find something was already knowing its exact title (header search) or
// waiting for it to trend. Deliberately progressive disclosure (a single
// small button, not a persistent filter bar) so the default trending glance
// stays exactly as clean as it was before this existed.
    ...
    <div>
      <div className={styles.browseBar}>
        <Button variant="secondary" size="sm" onClick={() => setBrowseOpen(v => !v)}>
          <SlidersHorizontal size={14} /> {browseOpen ? 'Back to Trending' : 'Browse by genre'}
        </Button>
      </div>

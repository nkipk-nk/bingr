...
export default function DetailPanel({ item, entry = {}, onBack, onSetStatus, onSetRating }) {
  ...
  useEffect(() => {
    setDetails(null); setProviders({}); setRecs([])
    Promise.all([
      type === 'movie' ? tmdb.movieDetails(item.id) : tmdb.tvDetails(item.id),
  ...
  // Streaming providers — prefer KE, fall back to US then GB
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 20, padding: '1.5rem' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', margin: '0 0 6px', lineHeight: 1.3 }}>{title}</h1>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 10 }}>
              {[year, type === 'tv' ? 'TV Series' : 'Movie', genres, extra].filter(Boolean).join(' · ')}
        ...
        <div style={{ padding: '1.25rem 1.5rem' }}>
          ...
          {flat.length || rent.length || buy.length ? (
            <>
              <ProviderChips items={flat} label="Stream" />
              <ProviderChips items={rent} label="Rent" />
              <ProviderChips items={buy} label="Buy" />
            </>
          ) : (
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              {details ? 'No streaming info available for your region.' : 'Loading...'}
            </div>
          )}
        </div>
      ...
      {recs.length > 0 && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 10 }}>
            {recs.map(r => (
              <div key={r.id} onClick={() => onBack(r)} style={{ cursor: 'pointer', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', transition: 'border-color 0.15s' }}>
                ...
                <div style={{ padding: '7px 9px 9px' }}>
                  <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', lineHeight: 1.35, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                    {r.title || r.name}
                  </div>

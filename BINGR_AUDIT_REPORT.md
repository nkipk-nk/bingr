...
| m3 | **Components defined inside components** (`react-hooks/static-components` ×10) remount on every parent render: `CardGrid` [App.jsx:311](src/App.jsx#L311), `ProviderChips` [DetailPanel.jsx:59](src/components/DetailPanel.jsx#L59), `UserRow` [FindPeople.jsx:37](src/components/FindPeople.jsx#L37), `Sel` [ExportPanel.jsx:23](src/components/ExportPanel.jsx#L23). | 4 files |
...
| m6 | Three timers stashed on `window`: `window._unTimer` ([AuthPage.jsx:41](src/pages/AuthPage.jsx#L41), [OnboardingModal.jsx:26](src/components/OnboardingModal.jsx#L26), [UsernamePrompt.jsx:21](src/components/UsernamePrompt.jsx#L21)) and `window._unameTimer` ([ProfilePage.jsx:26](src/pages/ProfilePage.jsx#L26)). Two components share `_unTimer` and will clobber each other. | 4 files |
| m7 | `FindPeople` fires a Supabase `ilike` query on **every keystroke** — no debounce, unlike the username checks which use 500 ms. | [FindPeople.jsx:22](src/components/FindPeople.jsx#L22) |
| m8 | Admin user search is **case-sensitive on `display_name`**: `u.display_name?.includes(userSearch.toLowerCase())` compares an un-lowercased field to a lowercased needle. | [AdminPanel.jsx:25](src/pages/AdminPanel.jsx#L25) |
| m9 | Feed tab badge `Feed (${feedHook.feed.length})` is **always 0** until the tab is opened, since `useFeed` has no mount-time load. | [App.jsx:304](src/App.jsx#L304) |
| m10 | `checkUsername` and the three inline availability checks use `.single()`, which returns a `PGRST116` error for the (expected) no-match case — generating console noise and 406s on the happy path. `.maybeSingle()` is the correct call. | [useProfile.js:72](src/hooks/useProfile.js#L72) +3 |
| m11 | Delete-account copy lists **"Your watchlist, watched list, and watchlist"** — "watchlist" twice; should be "watching". Also omits diary, lists, comments, and follows from the list of what's deleted. | [DeleteAccount.jsx:36](src/pages/DeleteAccount.jsx#L36) |
| m12 | `stats.js` hardcodes `AVG_MOVIE_RUNTIME = 110` / `AVG_EPISODE_RUNTIME = 42` and labels the result "Estimated watch time". TMDB returns real `runtime` per title — it's fetched in `DetailPanel` and discarded. | [stats.js:6-7](src/lib/stats.js#L6-L7) |
| m13 | `SupportersPage` sums `amount_kes` with no `.catch()` and no null guard — a single null amount produces `NaN` in "KES NaN raised total". | [SupportersPage.jsx:18](src/pages/SupportersPage.jsx#L18) |
| m14 | `bingr_comments` index is `(tmdb_id, media_type, created_at desc)` but the actual query filters on `status = 'visible'` too — the index doesn't cover it. | [supabase_comments.sql:68](supabase/supabase_comments.sql#L68) |
| m15 | Landing page claims **"500K+ Titles in database"** and "10 Point rating scale / 100% Free" as bingr stats — these are TMDB's numbers, presented as bingr's. Minor, but worth an asterisk. | [LandingPage.jsx:12](src/pages/LandingPage.jsx#L12) |
